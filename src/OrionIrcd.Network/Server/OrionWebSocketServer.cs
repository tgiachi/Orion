...
public sealed class OrionWebSocketServer : INetworkServer, IAsyncDisposable, IDisposable
{
    ...
    private readonly ILogger _logger = Log.ForContext<OrionWebSocketServer>();
    ...
    /// </summary>
    public ServerType ServerType => ServerType.WebSocket;
    ...
    private static async Task CloseAcceptedWebSocketAsync(WebSocket webSocket)
    {
        try
        {
            ...
            {
                using var cancellationTokenSource = new CancellationTokenSource(_acceptedSocketCloseTimeout);
    ...
    private async Task HandleRequestAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            client = new(webSocket, CreateRemoteEndPoint(context));
        ...
        catch (Exception ex)
        {
            ...
            OnException?.Invoke(this, new(ex, client));
        }
        finally
        {
    ...
    private bool TryGetShutdownToken(out CancellationToken shutdownToken)
    {
        ...
        var shutdownCancellationTokenSource = Volatile.Read(ref _shutdownCancellationTokenSource);
